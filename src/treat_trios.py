...
def show_launcher():
    ...
    pygame.display.set_caption("Candy Crush - Menu")
    ...
    while True:
        ...
        title = font.render("Candy Crush", True, (0, 0, 0))
        screen.blit(title, (130, 30))
        desc = font.render("Match 3 candies to win!", True, (0, 0, 0))
        screen.blit(desc, (70, 100))
...
def create_grid():
    # Initialize grid with random candies (1=Red, 2=Green, 3=Blue)
    grid = []
    for row in range(GRID_SIZE):
        new_row = []
        for col in range(GRID_SIZE):
